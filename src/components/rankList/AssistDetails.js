...
class AssistDetails extends Component {
  ...
  render () {
    ...
    const assistId = navigation.getParam('assistId', '')
    return (
      <Container>
        <Content style={Styles.contentStyle}>
          <View style={Styles.goalDetailsTop}>
            ...
            <View>
              <Text style={Styles.topTitleText}>助攻时间</Text>
            </View>
          </View>
          {/* 数据区域 */}
          <View style={Styles.goalDetailsContent}>
            <Query query={ALL_EVENTS} variables={{ condition: { assistId: assistId }, orderBy: 'MATCH_ID_ASC' }}>
              {({ data, error, loading }) => {
